...
const handleEditReview = (review) => {
  ...
  const starRatingStyles = `
    <style>
      ...
      .form-control:focus {
        border-color: #47845D;
        box-shadow: 0 0 0 0.2rem rgba(71, 132, 93, 0.25);
      }
      .form-label {
        color: #1E432E;
        font-weight: 500;
  ...
  // 創建星星評分HTML
  const createStarRatingHtml = (name, value) => {
    let html = `<div class="rating-edit-item">
      <span class="rating-edit-label">${name}</span>
      <div class="star-rating-edit">`;
    
    for (let i = 5; i >= 1; i--) {
      html += `
        <input type="radio" id="${name}_${i}" name="${name}" value="${i}" ${value === i ? 'checked' : ''} />
  ...
  Swal.fire({
    title: '<span style="color: #1E432E; font-weight: 700;">編輯評價</span>',
    html: `
      ${starRatingStyles}
      <div class="form-section">
        <div class="form-section-title"><i class="bi bi-pencil-fill me-2"></i>評價內容</div>
        <div class="mb-3">
          <label class="form-label">描述您的體驗</label>
          <textarea id="reviewText" class="form-control" rows="5" placeholder="請描述您的營地體驗...">${review.reviewText || ''}</textarea>
        </div>
        <div class="mb-3">
          <label class="form-label"><i class="bi bi-hand-thumbs-up-fill me-1"></i>優點</label>
          <textarea id="pros" class="form-control" rows="3" placeholder="有什麼特別喜歡的地方？">${review.pros || ''}</textarea>
        </div>
        <div class="mb-3">
          <label class="form-label"><i class="bi bi-hand-thumbs-down-fill me-1"></i>缺點</label>
          <textarea id="cons" class="form-control" rows="3" placeholder="有什麼需要改進的地方？">${review.cons || ''}</textarea>
        </div>
      </div>

      <div class="rating-edit-container">
        <div class="rating-header"><i class="bi bi-star-fill me-2"></i>評分</div>
        ${createStarRatingHtml('overallRating', review.overallRating)}
        ${createStarRatingHtml('cleanlinessRating', review.cleanlinessRating)}
        ${createStarRatingHtml('convenienceRating', review.convenienceRating)}
        ${createStarRatingHtml('friendlinessRating', review.friendlinessRating)}
      </div>
    `,
    showCancelButton: true,
    confirmButtonText: '確認更新',
    cancelButtonText: '取消',
    confirmButtonColor: '#47845D',
    ...
    preConfirm: () => {
      ...
      // 獲取評分
      const overallRating = parseInt(document.querySelector('input[name="overallRating"]:checked')?.value || review.overallRating);
      const cleanlinessRating = parseInt(document.querySelector('input[name="cleanlinessRating"]:checked')?.value || review.cleanlinessRating);
      const convenienceRating = parseInt(document.querySelector('input[name="convenienceRating"]:checked')?.value || review.convenienceRating);
      const friendlinessRating = parseInt(document.querySelector('input[name="friendlinessRating"]:checked')?.value || review.friendlinessRating);
...
const handleReply = (review) => {
  ...
  // 創建星星評分顯示
  const createStarRatingDisplay = (rating) => {
    let stars = '';
    for (let i = 1; i <= 5; i++) {
      stars += i <= rating ? '★' : '☆';
    }
  ...
  Swal.fire({
    title: '<span style="color: #1E432E; font-weight: 700;"><i class="bi bi-chat-dots me-2"></i>回覆評價</span>',
    html: `
      ${replyStyles}
      <div class="reply-dialog-container">
        <div class="original-review">
          <div class="original-review-header">
            <div>
              <div class="original-review-user">${review.userName || '匿名用戶'}</div>
              <div class="original-review-date">${formatDate(review.createdAt)}</div>
            </div>
            <div class="original-review-rating">${createStarRatingDisplay(review.overallRating)}</div>
          </div>
          <div class="original-review-content">${review.reviewText}</div>
          
          <div class="rating-details" style="display: flex; gap: 15px; margin-top: 10px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 120px;">
              <span style="font-weight: 500; color: #1E432E;">清潔程度: </span>
              <span style="color: #FFCB45;">${createStarRatingDisplay(review.cleanlinessRating)}</span>
            </div>
            <div style="flex: 1; min-width: 120px;">
              <span style="font-weight: 500; color: #1E432E;">便利程度: </span>
              <span style="color: #FFCB45;">${createStarRatingDisplay(review.convenienceRating)}</span>
            </div>
            <div style="flex: 1; min-width: 120px;">
              <span style="font-weight: 500; color: #1E432E;">友善程度: </span>
              <span style="color: #FFCB45;">${createStarRatingDisplay(review.friendlinessRating)}</span>
            </div>
          </div>
        </div>
        
        <div class="reply-form">
          <div class="reply-form-header"><i class="bi bi-chat-left-text me-2"></i>您的回覆</div>
          <label class="form-label">請輸入回覆內容</label>
          <textarea id="replyText" class="reply-textarea" placeholder="回覆此評價..."></textarea>
          
          <div class="reply-tips">
            <i class="bi bi-lightbulb me-2"></i>提示: 友善的回覆可以增加顧客的滿意度與信任感
          </div>
        </div>
      </div>
    `,
    showCancelButton: true,
    confirmButtonText: '<i class="bi bi-send me-1"></i>提交回覆',
    cancelButtonText: '<i class="bi bi-x me-1"></i>取消',
    confirmButtonColor: '#47845D',
    cancelButtonColor: '#6c757d',
    width: '600px',
    preConfirm: () => {
...
const handleEditReply = (review) => {
  ...
  Swal.fire({
    title: '<span style="color: #1E432E; font-weight: 700;"><i class="bi bi-pencil-fill me-2"></i>編輯回覆</span>',
    html: `
      ${editReplyStyles}
      <div class="edit-reply-container">
        <div class="original-review">
          ...
          <div class="original-reply">
            <div class="original-reply-header"><i class="bi bi-chat-left-text me-2"></i>目前回覆</div>
            <div class="original-reply-content">${review.replyText}</div>
          </div>
        </div>
        
        <div class="edit-reply-form">
          <div class="edit-reply-form-header"><i class="bi bi-pencil-square me-2"></i>修改回覆</div>
          <label class="edit-form-label">編輯您的回覆內容</label>
          <textarea id="editReplyText" class="edit-reply-textarea" placeholder="更新您的回覆...">${review.replyText || ''}</textarea>
          
          <div class="edit-reply-tips">
            <i class="bi bi-info-circle me-2"></i>修改回覆會保留原評價詳情及顯示「已編輯」標記
          </div>
        </div>
      </div>
    `,
    showCancelButton: true,
    confirmButtonText: '<i class="bi bi-check2 me-1"></i>更新回覆',
    cancelButtonText: '<i class="bi bi-x me-1"></i>取消',
    confirmButtonColor: '#47845D',
    cancelButtonColor: '#6c757d',
    width: '600px',
